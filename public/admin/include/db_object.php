...
class DB_Object
{
    ...
    public static function find_by_quotation_id($id) {
        $the_result_array = static::find_by_query("SELECT * FROM " . static::$db_table . " WHERE quotation_id = $id ORDER BY quotation_id DESC  LIMIT 1 ");
        return !empty($the_result_array) ? array_shift($the_result_array) : false;
    }
    ...
    public function save_quotation() {
        return isset($this->quotation_id) ? $this->update_quotation() : $this->create();
    }
    ...
    public function update_quotation() {
        ...
        $sql = "UPDATE " . static::$db_table . " SET ";
        $sql .= implode(", ", $properties_pairs);
        $sql .= " WHERE quotation_id = " . $db->escape_string($this->quotation_id);
